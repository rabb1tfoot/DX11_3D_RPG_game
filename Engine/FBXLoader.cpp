...
void CFBXLoader::GetTangent(FbxMesh * _pMesh, tContainer * _pContainer
...
Vec4 CFBXLoader::GetMtrlData(FbxSurfaceMaterial * _pSurface, const char * _pMtrlName, const char * _pMtrlFactorName)
{
	...
	Vec4 vRetVal = Vec4((float)(vMtrl.mData[0] * dFactor), (float)(vMtrl.mData[1] * dFactor), (float)(vMtrl.mData[2] * dFactor), (float)dFactor);
	return vRetVal;
}
...
void CFBXLoader::LoadTexture()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			//노말택스처가 있어야 로딩한다.
			if(wcscmp(strPath.c_str(), L"") || wcscmp(strFileName.c_str(), L""))
				CResMgr::GetInst()->Load<CTexture>(strFileName, strPath);

			//노말택스처가 없는경우 직접만든 노말맵으로 로딩한다.
			else
			{
				strPath = CPathMgr::GetRelativePath(m_vecContainer[i].vecMtrl[j].strDiff.c_str());
				strFileName = CPathMgr::GetFileName(m_vecContainer[i].vecMtrl[j].strDiff.c_str());

				//파일이름을 _normal을 붙여서 로딩 
				//EX) strPath L"FBX\\AGRON.fbm\\agronn_red.png" -> ...aronn_red_normal.png
				//EX) strFileName L"agronn_red" -> agronn_red_normal.png
				for (UINT i = strPath.size(); i > 0; --i)
				{
					if (strPath[i] == '.')
					{
						strPath.erase(i, strPath.size());
						strPath += L"_normal.png";
						break;
					}
				}
				strFileName += L"_normal";
...
void CFBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			wstring strTexName = strKey;
			if (strKey[0] != L'F')
			{
				strKey = L"";
				strKey += GetMeshDataName();
				strKey[strKey.size()-1] = 'm';
				strKey += L"\\";
				strKey += strTexName;
				strKey += L".png";

				m_vecContainer[i].vecMtrl[j].ChangeDiff(strKey);
			}
			...
			//자체제작normal맵이 있는경우 추가해준다.
			//EX) strFileName L"agronn_red" -> L"agronn_red_normal"
			...
			if(strKey[0] != L'F')
			{
				...
				m_vecContainer[i].vecMtrl[j].ChangeNormal(strKey);
			}

			CResPtr<CTexture> pTex2 = CResMgr::GetInst()->FindRes<CTexture>(strKey);
			if (NULL == pTex && NULL != pTex2)
			{
				//자체제작 normal맵은 체크해둔다. 쉐이더에서 따로 동작
				int CheckOurNormalMap = 1;
				pMaterial->SetData(SHADER_PARAM::TEX_1, &pTex2);
				pMaterial->SetData(SHADER_PARAM::INT_0, &CheckOurNormalMap);
			}
			...
			CResMgr::GetInst()->AddRes<CMaterial>(pMaterial->GetPath(), pMaterial);
		}
	}
}
